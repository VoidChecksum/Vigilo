...
function getEmptyResultHint(pattern: string, lang: CliLanguage): string | null {
  ...
  if (["javascript", "typescript", "tsx", "solidity"].includes(lang)) {
...
export const ast_grep_search: ToolDefinition = tool({
  description:
    "Search code patterns across filesystem using AST-aware matching. Supports 25 languages including Solidity. " +
    "Use meta-variables: $VAR (single node), $$$ (multiple nodes). " +
    "IMPORTANT: Patterns must be complete AST nodes (valid code). " +
    "For functions, include params and body: 'function $NAME($$$) { $$$ }' not 'function $NAME'. " +
    "Solidity examples: 'function $NAME($$$) external', 'require($COND, $MSG)', 'msg.sender'",
  args: {
    pattern: tool.schema.string().describe("AST pattern with meta-variables ($VAR, $$$). Must be complete AST node."),
    lang: tool.schema.enum(CLI_LANGUAGES).describe("Target language (e.g., 'solidity', 'rust', 'typescript')"),
...
export const ast_grep_replace: ToolDefinition = tool({
  description:
    ...
    "Example: pattern='require($COND)' rewrite='require($COND, \"error message\")'",
  args: {
