...
import { ALLOWED_AGENTS, CALL_VIGILO_AGENT_DESCRIPTION } from "./constants"
import type { CallVigiloAgentArgs } from "./types"
...
export function createCallVigiloAgent(
  ctx: PluginInput,
  ...
  const description = CALL_VIGILO_AGENT_DESCRIPTION.replace("{agents}", agentDescriptions)
  ...
  return tool({
    ...
    async execute(args: CallVigiloAgentArgs, toolContext) {
      const toolCtx = toolContext as ToolContextWithMetadata
      log(`[call_vigilo_agent] Starting with agent: ${args.subagent_type}, background: ${args.run_in_background}`)
...
async function executeBackground(
  ...
  try {
    ...
    log("[call_vigilo_agent] parentAgent resolution", {
...
async function executeSync(
  ...
  if (args.session_id) {
    log(`[call_vigilo_agent] Using existing session: ${args.session_id}`)
    const sessionResult = await ctx.client.session.get({
      path: { id: args.session_id },
    })
    if (sessionResult.error) {
      log(`[call_vigilo_agent] Session get error:`, sessionResult.error)
      return `Error: Failed to get existing session: ${sessionResult.error}`
    }
    sessionID = args.session_id
  } else {
    log(`[call_vigilo_agent] Creating new session with parent: ${toolContext.sessionID}`)
    const parentSession = await ctx.client.session.get({
      path: { id: toolContext.sessionID },
    }).catch((err) => {
      log(`[call_vigilo_agent] Failed to get parent session:`, err)
      return null
    })
    log(`[call_vigilo_agent] Parent session dir: ${parentSession?.data?.directory}, fallback: ${ctx.directory}`)
    ...
    if (createResult.error) {
      log(`[call_vigilo_agent] Session create error:`, createResult.error)
      return `Error: Failed to create session: ${createResult.error}`
    }

    sessionID = createResult.data.id
    log(`[call_vigilo_agent] Created session: ${sessionID}`)
  }
  ...
  log(`[call_vigilo_agent] Sending prompt to session ${sessionID}`)
  log(`[call_vigilo_agent] Prompt text:`, args.prompt.substring(0, 100))
  ...
  try {
    ...
    log(`[call_vigilo_agent] Prompt error:`, errorMessage)
  ...
  log(`[call_vigilo_agent] Prompt sent, polling for completion...`)
  ...
  while (Date.now() - pollStart < MAX_POLL_TIME_MS) {
    if (toolContext.abort?.aborted) {
      log(`[call_vigilo_agent] Aborted by user`)
      return `Task aborted.\n\n<task_metadata>\nsession_id: ${sessionID}\n</task_metadata>`
    }
    ...
    if (currentMsgCount > 0 && currentMsgCount === lastMsgCount) {
      ...
      if (stablePolls >= STABILITY_REQUIRED) {
        log(`[call_vigilo_agent] Session complete, ${currentMsgCount} messages`)
        break
      }
    } else {
      stablePolls = 0
  ...
  if (Date.now() - pollStart >= MAX_POLL_TIME_MS) {
    log(`[call_vigilo_agent] Timeout reached`)
  ...
  if (messagesResult.error) {
    log(`[call_vigilo_agent] Messages error:`, messagesResult.error)
    return `Error: Failed to get messages: ${messagesResult.error}`
  }

  const messages = messagesResult.data
  log(`[call_vigilo_agent] Got ${messages.length} messages`)
  ...
  if (relevantMessages.length === 0) {
    log(`[call_vigilo_agent] No assistant or tool messages found`)
    log(`[call_vigilo_agent] All messages:`, JSON.stringify(messages, null, 2))
    return `Error: No assistant or tool response found\n\n<task_metadata>\nsession_id: ${sessionID}\n</task_metadata>`
  }

  log(`[call_vigilo_agent] Found ${relevantMessages.length} relevant messages`)
  ...
  log(`[call_vigilo_agent] Got response, length: ${responseText.length}`)
