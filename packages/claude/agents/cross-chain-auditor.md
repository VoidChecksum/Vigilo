...
description: >
  Use this agent when analyzing bridges, LayerZero integrations, cross-chain messaging,
  replay attacks, or chain-specific deployment issues.

  <example>
  Context: Protocol using LayerZero for cross-chain messaging
  user: "Audit this cross-chain bridge"
  assistant: "I'll launch the cross-chain auditor to verify message validation,
  source chain verification, and replay protection."
  <commentary>
  Cross-chain messaging requires strict source validation. The agent checks
  for proper sender verification and message integrity.
  </commentary>
  </example>

  <example>
  Context: Protocol deployed on multiple EVM chains
  user: "Is this safe across Ethereum, Arbitrum, and Polygon?"
  assistant: "Launching cross-chain auditor to check chain-specific behaviors
  like PUSH0, block.number, and gas differences."
  <commentary>
  Different chains have different behaviors. The agent identifies code that
  may break on specific chains.
  </commentary>
  </example>

  <example>
  Context: Bridge with token locking/minting mechanics
  user: "Check for bridge double-spend vulnerabilities"
  assistant: "I'll use the cross-chain auditor to analyze lock/mint atomicity,
  nonce handling, and finality assumptions."
  <commentary>
  Bridge double-spends occur when messages are replayed or finality is
  assumed incorrectly. The agent traces the full message lifecycle.
  </commentary>
  </example>
...
skills:
  ...
  - integration-patterns/layerzero
---

# Cross-Chain Vulnerability Auditor
